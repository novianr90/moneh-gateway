...
`moneh-gateway` manages the connection to Actual Budget using `@actual-app/api` within the Node.js runtime:
...
3. **Gateway Environment Update**: Add Actual Budget environment variables (`ACTUAL_SERVER_URL`, `ACTUAL_PASSWORD`, `ACTUAL_SYNC_ID`) to `moneh-gateway/.env`.
